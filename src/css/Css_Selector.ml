...
module Element = struct
  ...
  type t =
    ...
    | `legend | `li | `main | `map | `mark | `meta | `meter | `nav | `noscript | `option
    | `object_ [@bs.as "object"] | `ol | `optgroup | `output | `p | `param | `picture | `pre
    | `progress | `q | `rb | `rp | `rt | `rtc | `ruby | `s | `samp | `script | `section | `select
    | `source | `span | `slot | `small | `strong | `style | `sub | `summary | `sup | `table | `tbody
